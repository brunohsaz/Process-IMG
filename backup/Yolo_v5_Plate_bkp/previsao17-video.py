...
# ---------------------- EXECUÇÃO COM VÍDEO ----------------------
video_path = str(base_dir / 'videos/video-1.mp4')
...
while True:
    ...
    for i, placa_proc in enumerate(placas_processadas):
        ...
        if rois_caracteres:
           ...
           # só faz requisição se passou o intervalo e texto mudou
            agora = time.time()
            if len(texto) == 7 and texto != texto_anterior and (agora - ultimo_request) >= intervalo_request:
