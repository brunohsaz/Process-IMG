...
def train(opt, show_number = 2, amp=False):
    ...
    print(f"Total imagens de treino: {len(train_dataset)}")
    print(f"Total imagens de validação: {len(valid_dataset)}")
    ...
    while(True):
        ...
        if amp:
            with autocast():
                ...
                print(f"Iter {i}: Labels deste batch -> {labels}")
        ...
        save_path = os.path.join(opt.model_save_directory, 'custom_model_final.pth')
