...
def enviar_requisicao(texto):
    global ultima_placa_reconhecida, intervalo_request

    try:
        url = "https://localhost:4040/api/veiculos/placa/" + texto
        resposta = requests.get(url, verify=False, timeout=5)
        if resposta.status_code == 200:
            dados = resposta.json()
            ultima_placa_reconhecida = dados.get("placa")
            if dados.get("acesso"):
                intervalo_request = 10
            else:
                intervalo_request = 5
        else:
...
# ---------------------- EXECUÇÃO COM VÍDEO ----------------------
video_path = str(base_dir / 'videos/video-4.mp4')
...
texto_anterior = ""
ultimo_request = 0
intervalo_request = 5  # padrão inicial
placa_reconhecida = None
...
while True:
    ...
    # se ainda estiver dentro do intervalo, apenas exibe o frame
    ...
    for i, placa_proc in enumerate(placas_processadas):
        ...
        if deve_enviar:
            ...
            print("Reconhecida:", texto)

            # faz a requisição sem travar o vídeo
            threading.Thread(target=enviar_requisicao, args=(texto,), daemon=True).start()
